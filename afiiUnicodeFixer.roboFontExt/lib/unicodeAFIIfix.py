...
def getUnicodeByAfii (glyph_afii):
	for g in cyrlTab:
		if g[afiiNumber] == glyph_afii:
			return g[uniNumber]
def getUnicodeByAfii_Int(glyph_affi):
	return int(getUnicodeByAfii(glyph_affi),16)
...
for glyph in font:
	if 'afii' in glyph.name and not glyph.unicode and '.' not in glyph.name and '_' not in glyph.name:
		glyph.unicode = getUnicodeByAfii_Int(glyph.name)